...
for i, house_bus in enumerate(houses, start=1):
    dss.text(f"new pvsystem.Solar{i} bus1={house_bus} phases=3 conn=wye "
              f"kv=0.415 kva=6 pmpp=6 irradiance=1 %cutin=0 %cutout=0")
...
print(df)
df.to_csv(os.path.join(OUTPUTS_DIR, "results_solar.csv"), index=False)

mt.plot(df["bus"], df["voltage_pu"], marker="o", label="With solar at bus6")
...
mt.title("Voltage Profile With Solar PV Added")
mt.legend()
mt.savefig(os.path.join(OUTPUTS_DIR, "voltage_profile_with_solar.png"), dpi=350)
mt.show()