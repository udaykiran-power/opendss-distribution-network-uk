...
for i, house_bus in enumerate(houses, start=1):
    dss.text(f"new load.house{i} bus1={house_bus} phase=3 conn=wye "
...
print(df)
df.to_csv(os.path.join(OUTPUTS_DIR, "results_baseline.csv"), index=False)

mt.plot(df["bus"], df["voltage_pu"], marker="o")
...
mt.title("Voltage Profile Along UK LV Feeder")
mt.legend()
mt.savefig(os.path.join(OUTPUTS_DIR, "voltage_profile_baseline.png"), dpi=350)
mt.show()